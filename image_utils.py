...
def save_img_and_pred(depth, save_path):
    """Plot an image and a corresponding prediction next to each other, then save the plot."""
    depth_gray = depth_to_grayscale(depth=depth)
    darkened_image = np.clip(-depth_gray-100, 0, 255).astype(np.uint8)
    # pred = np.transpose(depth_gray, (1, 2, 0))
